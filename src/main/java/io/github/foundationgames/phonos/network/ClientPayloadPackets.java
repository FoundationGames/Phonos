        ...
        ClientPlayNetworking.registerGlobalReceiver(Phonos.id("jukebox_song_by_id"), (client, handler, buf, sender) -> {
            ...
            client.execute(() -> {
                if(client.player.getEntityWorld() instanceof ClientWorld) {
                    ...
                    if(sound != null) soundI = new PositionedSoundInstance(sound, SoundCategory.RECORDS, 1.8f, 1.0f, SoundInstance.createRandom(), false, 0, SoundInstance.AttenuationType.LINEAR, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, false);
